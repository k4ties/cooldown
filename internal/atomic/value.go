...
// Load tries to load a value from parent atomic.Value. If value is nil, zero T
// and false is returned. Then, it tries to type assert the value to T and
...
// Swap swaps current Value val with the specified one. If currently Value has
// not val, zero T and false is returned.
